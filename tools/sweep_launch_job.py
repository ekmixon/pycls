...
def prt(*args, **kwargs):
    ...
    print(f"[{str(datetime.now())}]", *args, flush=True, **kwargs)
...
def main():
    ...
    job_id = os.environ["SLURM_ARRAY_JOB_ID"]
    task_id = os.environ["SLURM_ARRAY_TASK_ID"]
    prt(f"Job array main job ID: {job_id}")
    prt(f"Job array task ID (index): {task_id}")
    prt(f"Running job on: {str(os.uname())}")
    # Load what we need
    run_os_cmd("module purge")
    run_os_cmd("module load anaconda3")
    run_os_cmd("source deactivate")
    run_os_cmd(f"source activate {args.conda_env}")
    # Get cfg_file to use
    ...
    prt(f"Dumped SLURM job info to {slurm_file}")
    # Set PYTHONPATH to pycls copy for sweep
    os.environ["PYTHONPATH"] = args.pycls_dir
    prt(f"Using PYTHONPATH={args.pycls_dir}")
    ...
    p.wait()
    prt(f"Completed waiting. Return code for job: {p.returncode}")
    if p.returncode != 0:
        ...
        prt(f"Retry count for job: {retry_count}")
